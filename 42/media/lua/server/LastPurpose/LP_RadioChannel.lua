...
LastPurposeRadio.CANDIDATE_COUNT = 10
LastPurposeRadio.BROADCAST_SLOT_HOURS = 0.5
...
local function getOccupiedFrequencies(scriptManager)
    ...
    if not ok then
        print("[LastPurpose] ADVERTENCIA: no se pudo leer la lista de canales del motor")
    end

    -- DynamicRadio tambien contiene canales declarados por otros mods que aun
    -- no ejecutaron su turno de registro en OnLoadRadioScripts.
...
local function getFrequency(occupied)
    local worldData = getGameTime():getModData()
    local saved = worldData[LastPurposeRadio.MODDATA_KEY]
    ...
    if frequency then
        if isValidFrequency(saved) and occupied[saved] then
            LastPurpose.debugPrint("Colision de radio detectada en " .. tostring(saved / 1000) .. " MHz; buscando otra frecuencia")
        end
        worldData[LastPurposeRadio.MODDATA_KEY] = frequency
    end
    return frequency
end
...
local function validCandidateList(list, occupied, selected)
    ...
    for frequency = LastPurposeRadio.MIN_FREQUENCY, LastPurposeRadio.MAX_FREQUENCY, LastPurposeRadio.FREQUENCY_STEP do
        if not occupied[frequency] or frequency == selected then available = available + 1 end
    end
    if type(list) ~= "table" or #list ~= math.min(LastPurposeRadio.CANDIDATE_COUNT, available) then return false end
...
local function createCandidateList(occupied, selected)
    ...
    local list = { selected }
    while #list < LastPurposeRadio.CANDIDATE_COUNT and #pool > 0 do
...
function LastPurposeRadio.onLoadRadioScripts(scriptManager)
    local ok, err = pcall(function()
        ...
        local frequency = getFrequency(occupied)
        ...
        LastPurpose.debugPrint("Canal narrativo registrado en " .. tostring(frequency / 1000) .. " MHz")
    end)
    if not ok then
        -- Preferimos quedarnos sin canal narrativo a tirar la carga de radio
        -- completa si el motor cambio alguna de estas firmas.
        print("[LastPurpose] ERROR registrando el canal de radio: " .. tostring(err))
    end
end

local function createBroadcast(worldAgeHours)
    local ok, result = pcall(function()
        local broadcast = RadioBroadCast.new("LP-HEIST-" .. tostring(math.floor(worldAgeHours / LastPurposeRadio.BROADCAST_SLOT_HOURS)), -1, -1)
    ...
    if not ok then
        print("[LastPurpose] ERROR construyendo la transmision: " .. tostring(result))
        return nil
    end
    return result
end
...
function LastPurposeRadio.onEveryTenMinutes()
    ...
    local slot = math.floor(gameTime:getWorldAgeHours() / LastPurposeRadio.BROADCAST_SLOT_HOURS)
    if LastPurposeRadio.lastBroadcastSlot == slot then return end

    local broadcast = createBroadcast(gameTime:getWorldAgeHours())
    if not broadcast then return end -- reintenta en el siguiente ciclo de 10 min

    local ok, err = pcall(function() LastPurposeRadio.channel:setAiringBroadcast(broadcast) end)
    if not ok then
        print("[LastPurpose] ERROR emitiendo la transmision: " .. tostring(err))
        return
    end
